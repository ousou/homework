...
def learn(env,
    ...
    q_target = q_func(obs_tp1_float, num_actions, scope="q_func_target", reuse=False)
    q_func_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='q_func')
    target_q_func_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='q_func_target')

    y_t = rew_t_ph + (1 - done_mask_ph) * gamma * tf.reduce_max(q_target, axis = 1)
    action_mask = tf.one_hot(act_t_ph, num_actions)
    current_q_val = tf.reduce_sum(tf.multiply(action_mask, q), 1)
